...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Hubi in school la soo galay. Haddii kale, ku celi bogga hore ee login-ka.
  ...
  async function fetchDashboardData(schoolCode) {
    try {
      setLoading(true);

      // Dhammaan xogta waxaa lagu soo qaadanayaa oo keliya school-ka hadda soo galay.
      // scopedDocs = getDocs(collection) laakiin filter-gareysan schoolCode.
      const scopedDocs = (name) =>
        getDocs(query(collection(db, name), where("schoolCode", "==", schoolCode)));

      const studentsSnap = await scopedDocs("students");
      ...
      const teachersSnap = await scopedDocs("teachers");
      ...
      const cashierSnap = await scopedDocs("cashier");
      ...
      const paymentsSnap = await scopedDocs("payments");
      ...
      const examsSnap = await scopedDocs("exams");
      ...
      const resultsSnap = await scopedDocs("results");
      ...
      const attendanceSnap = await scopedDocs("attendance");
      ...
      const messagesSnap = await scopedDocs("messages");
      ...
      const shiftsSnap = await scopedDocs("shifts");
        ...
        <div style={{ padding: "26px 30px" }}>
          ...
          >
            ...
            <h1 style={{ margin: 0, fontSize: 30, fontWeight: 800, lineHeight: 1.15 }}>
              {(getSchoolName() || "SCHOOL").toUpperCase()}!
            </h1>
